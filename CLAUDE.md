...
- **Headless CLI:** `glimpse status|log|branches|info` (read) and `glimpse cl ls|add|mv|rm|active|commit` (changelists), each with `--json` and `-C <dir>`, plus `glimpse --help` / `--version`. Write-action parity with the GUI, packaging and WSL distribution are still open (#103).
