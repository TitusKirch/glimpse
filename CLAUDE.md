...
- **Headless CLI:** every GUI **read** view has a subcommand — `glimpse status|diff|log|show|history|blame|branches|stashes|reflog|worktrees|submodules|sparse|stats|info` — plus the working-tree **writes** `glimpse stage|unstage|discard|commit|amend` (`crates/glimpse-cli/src/write.rs`) and `glimpse cl ls|add|mv|rm|active|commit` (changelists), each with `--json` and `-C <dir>`, plus `glimpse --help` / `--version`. Both globals work **before** the subcommand as well as after it (`glimpse -C sub status`): `globals_after_the_subcommand` in `crates/glimpse-cli/src/lib.rs` rewrites argv so the one parser sees either spelling, and `claims` looks past them too — otherwise the single installed binary would read `-C` as a path and open a window. A subcommand counts as shipped only once it is in `SUBCOMMANDS`, `glimpse --help` **and** the README; three `cargo` tests enforce that (the README guard matches on a word boundary, so a rename that keeps the old name as a prefix fails it). The remaining write actions (branch, tags, remotes, stash, fetch/pull/push, rebase, bisect, conflict resolution), packaging and WSL distribution are still open (#103).
