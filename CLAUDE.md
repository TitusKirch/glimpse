...
- **Everything destructive states its confirmation, and the rule is one rule.** Naming the subject **is** the confirmation — `discard <file>`, `branch delete <name>`, `stash drop <stash>` need no flag, which is why `stash drop` refuses to assume `stash@{0}` the way bare `git stash drop` does. An action naming **no** subject carries `--force`: `discard --all --force`, and `reset --hard` **only when the tree is dirty** (a clean tree has nothing to consent to). `branch delete --force` is the one flag that means something extra — deleting the ref is what was named, losing commits no other ref holds is not, so it maps to `-D` while the plain form stays `-d`.
- **A verb that _begins_ an operation refuses while one is open** (`refs::in_progress` → `MERGE_HEAD` / `CHERRY_PICK_HEAD` / `REVERT_HEAD`): `branch merge`, `cherry-pick`, `revert`, and `discard --all --force` all ask first, so the refusal lands before anything is touched and names the paths still in dispute. `REBASE_HEAD` is deliberately **not** in that set — a stopped rebase has no glimpse subcommand to finish it with yet, so refusing on it would refuse work with no way out; it belongs to the slice that adds `rebase` (#103). `reset --hard` is likewise not refused: it _clears_ those refs, so it concludes the operation instead of hiding it, and it is currently the only route out of a stopped one.
- **The refs commands read their outcome back from the ref store**, the way the working-tree ones read theirs back from `status`: `current_branch`, `branch_names`, `tag_names`, `remote_names`, `commits_since` and `remote_url` on `Repo` exist for exactly that, and are cheap probes rather than `info()`, which runs half a dozen git commands to answer one question. A cherry-pick or revert writes **new** commits, so the report carries the hashes git actually wrote, never the ones it was handed; `push_tags` uses `--porcelain` because the human format goes to stderr, which the engine returns only on failure (a successful `git push --tags` used to hand its caller an empty string).
