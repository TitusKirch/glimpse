...
- **Headless CLI:** every GUI **read** view has a subcommand — `glimpse status|diff|log|show|history|blame|branches|stashes|reflog|worktrees|submodules|sparse|stats|info` — alongside `glimpse cl ls|add|mv|rm|active|commit` (changelists), each with `--json` and `-C <dir>`, plus `glimpse --help` / `--version`. A subcommand counts as shipped only once it is in `SUBCOMMANDS`, `glimpse --help` **and** the README; two `cargo` tests enforce those last two. **Write** actions, packaging and WSL distribution are still open (#103).
